...
import fsSync, {
	...
	type Mode,
	type WriteFileOptions as FsSyncWriteFileOptions,
	type OpenMode,
} from "node:fs"; // Consolidate fsSync imports and add WriteFileOptions
...
import type { Mock, Mocked, SpyInstance } from "vitest";
import {
	Chopup,
	INPUT_SEND_ERROR,
...
vi.mock("tree-kill", () => {
	const mockFn = vi.fn((pid, signal, callback) => {
		console.log(
			`[MOCK_TREEKILL_CORE] Mock called for PID: ${pid}, Signal: ${signal}`,
		);
...
const getTestSocketPath = () =>
	path.join(
		TEST_SOCKET_DIR,
		`test-core-${Date.now()}-${Math.random().toString(36).substring(7)}.sock`,
	);
...
let writeFileSpy: SpyInstance<
	[
		...
		// biome-ignore lint/suspicious/noExplicitAny: Complex type for spy options is overly burdensome
		options?: any, // Simplified to any for pragmatic mocking
	],
	Promise<void>
>;
let mkdirSpy: SpyInstance<
	[
		...
		options?:
			| Mode
			| (MakeDirectoryOptions & { recursive?: boolean | undefined })
			| null
			| undefined,
	],
	string | undefined
...
class FakeChildProcess extends EventEmitter {
	// Copied from chopup.test.ts for consistency
	pid = 12345;
	stdout: EventEmitter;
	stderr: EventEmitter;
	...
	constructor(hasStdin = true) {
		...
		if (hasStdin) {
			this.stdin = {
				write: vi.fn((data, cb) => {
					console.log(
						`[FAKE_CHILD_STDIN_WRITE] Mock write called. Data: ${data?.toString().substring(0, 20)}. CB exists: ${!!cb}`,
					);
					if (cb) {
...
function makeChopupWithMocks(
	socketPath: string,
	fakeChildInstance?: FakeChildProcess,
) {
	const spawnFn = vi
		.fn()
		.mockImplementation(() => fakeChildInstance || new FakeChildProcess());
...
describe("Chopup core logic", () => {
	...
	const connectClientWithRetries = async (
		socketPath: string,
		retries = 3,
		delay = 100,
	): Promise<net.Socket> => {
			...
			try {
				...
				await new Promise<void>((resolve, reject) => {
					const timer = setTimeout(() => {
						...
						reject(
							new Error(
								`[TEST_CORE_CLIENT] Connection attempt ${i + 1} timed out after 2s`,
							),
						);
					}, 2000); // 2-second timeout for each connection attempt
					currentClient?.once("connect", () => {
						clearTimeout(timer);
						resolve();
					});
				...
				console.log(
					`[TEST_CORE_CLIENT] Connected successfully to ${socketPath} on attempt ${i + 1}`,
				);
				if (!currentClient) {
					// This case should ideally not be reached if the promise resolved successfully
					throw new Error(
						"[TEST_CORE_CLIENT] Client is unexpectedly null after successful connection promise.",
					);
				}
				...
				console.warn(
					`[TEST_CORE_CLIENT] Connection attempt ${i + 1} to ${socketPath} failed: ${error.message}. Retrying in ${delay}ms...`,
				);
				if (i === retries - 1) {
					console.error(
						`[TEST_CORE_CLIENT] All ${retries} connection attempts to ${socketPath} failed.`,
					);
		...
		throw new Error(
			"[TEST_CORE_CLIENT] Should not reach here - all connection retries exhausted.",
		);
	};
	...
	afterEach(async () => {
		...
		if (currentClient && !currentClient.destroyed) {
			console.log(
				`[TEST_CLEANUP_CORE] Destroying client for ${currentTestSocketPath}`,
			);
			currentClient.destroy();
		...
		if (chopup) {
			...
			if (server?.listening) {
				...
				console.log(
					`[TEST_CLEANUP_CORE] Closing IPC server for ${currentTestSocketPath}`,
				);
				await new Promise<void>((resolve, reject) => {
					server.close((err) => {
						if (err) {
							console.error(
								`[TEST_CLEANUP_CORE] Error closing server for ${currentTestSocketPath}:`,
								err,
							);
							reject(err); // Propagate error if closing fails
						} else {
							console.log(
								`[TEST_CLEANUP_CORE] IPC server closed for ${currentTestSocketPath}`,
							);
							resolve();
						}
					});
			...
			if (child?.pid && currentFakeChild && !currentFakeChild.killed) {
				// Used optional chaining
				console.log(
					`[TEST_CLEANUP_CORE] Killing fake child PID ${child.pid} for ${currentTestSocketPath}`,
				);
				...
				if (treeKillMock) {
					...
					if (typeof pid !== "number") {
						throw new Error(
							`[TEST_CLEANUP_CORE] Expected child.pid to be a number, but got ${typeof pid}`,
						);
					}
					await new Promise<void>((resolveTreeKill, rejectTreeKill) => {
						// Renamed resolve to avoid conflict
						treeKillMock(pid, "SIGTERM", (err: Error | undefined) => {
							// Pass guarded pid
							if (err) rejectTreeKill(err);
							else resolveTreeKill();
						});
					});
				} else {
		...
		try {
			...
			console.log(
				`[TEST_CLEANUP_CORE] Successfully unlinked socket ${currentTestSocketPath}`,
			);
		} catch (err: unknown) {
			const error = err as NodeJS.ErrnoException;
			if (error.code !== "ENOENT") {
				console.warn(
					`[TEST_CLEANUP_CORE] Could not unlink socket ${currentTestSocketPath}:`,
					error.message,
				);
			}
		}
	...
	it.skip("send-input handler writes to child stdin and responds", async () => {
		...
		await new Promise<void>((resolve, reject) => {
			...
			const timer = setTimeout(
				() =>
					reject(
						new Error("[TEST_CORE_CLIENT] Connection timed out (send-input)"),
					),
				5000,
			);
			if (!currentClient) {
				// Null check for currentClient
				clearTimeout(timer);
				return reject(
					new Error("currentClient is null before 'connect' event"),
				);
			}
			currentClient.once("connect", () => {
				clearTimeout(timer);
		...
		const responsePromise = new Promise<string>((resolve, reject) => {
			...
			const timer = setTimeout(
				() =>
					reject(
						new Error("[TEST_CORE_CLIENT] Response timed out (send-input)"),
					),
				10000,
			); // Shorter timeout
			if (!currentClient) {
		...
		await new Promise<void>((resolveWrite, rejectWrite) => {
			...
			currentClient.write(
				JSON.stringify({ command: SEND_INPUT_COMMAND, input: "abc" }),
				(err) => {
					if (err) {
		...
		expect(currentFakeChild?.stdinWriteMock).toHaveBeenCalledWith(
			"abc\n",
			expect.any(Function),
		);
	...
	it.skip("request-logs command triggers chopLog and responds", async () => {
		...
		await new Promise<void>((resolve, reject) => {
			const timer = setTimeout(
				() =>
					reject(
						new Error("[TEST_CORE_CLIENT] Connection timed out (request-logs)"),
					),
				5000,
			);
			if (!currentClient) {
		...
		const responsePromise = new Promise<string>((resolve, reject) => {
			...
			const timer = setTimeout(
				() =>
					reject(
						new Error("[TEST_CORE_CLIENT] Response timed out (request-logs)"),
					),
				5000,
			);
			if (!currentClient) {
	...
	it.skip("send-input returns error if no child process", async () => {
		...
		await new Promise<void>((resolve, reject) => {
			const timer = setTimeout(
				() =>
					reject(
						new Error(
							"[TEST_CORE_CLIENT] Connection timed out (send-input-no-child)",
						),
					),
				5000,
			);
			...
			currentClient.once("error", (err) => {
				clearTimeout(timer);
				console.error(
					"[TEST_CORE_CLIENT_CONNECT_ERROR send-input-no-child]:",
					err,
				);
				reject(err);
			});
		...
		const responsePromise = new Promise<string>((resolve, reject) => {
			...
			const timer = setTimeout(
				() =>
					reject(
						new Error(
							"[TEST_CORE_CLIENT] Response timed out (send-input-no-child)",
						),
					),
				5000,
			);
			...
			currentClient.once("data", (data) => {
				...
				const responseData = data.toString();
				console.log(
					`[TEST_CORE_CLIENT_DATA send-input-no-child]: ${responseData}`,
				);
				resolve(responseData);
			});
			currentClient.once("error", (err) => {
		...
		await new Promise<void>((resolveWrite, rejectWrite) => {
			...
			currentClient.write(
				...
				(err) => {
					if (err) {
						console.error(
							"[TEST_CORE_CLIENT_WRITE_ERROR send-input-no-child]:",
							err,
						);
						return rejectWrite(err);
					}
		...
		if (currentFakeChild) {
			// If a fake child was associated (even if chopup.childProcess was later nulled)
