...
import type { Readable, Writable, PassThrough as PassThroughType } from 'node:stream';
...
export class FakeChildProcess extends EventEmitter {
    ...
    async exit(code: number | null = 0, signal: NodeJS.Signals | null = null): Promise<void> {
    ...
    getStdinContent(): Promise<string> {
        return new Promise((resolve) => {
            let content = '';
            this._stdinPT.on('data', (chunk) => { content = content + chunk.toString(); });
